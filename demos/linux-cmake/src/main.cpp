...
class NonlinearModel1 : public BayesianFiltering::StateSpaceNonlinearModelBase
{
...
public:
    ...
    // Necessary only for EKF or other filters requiring such derivatives
...
class NonlinearModel2 : public BayesianFiltering::StateSpaceNonlinearModelBase
{
    /* Process:  tracking x y Euclidian coordinates from rho and alpha polar coordinates.
        x1(k) = x1(k-1) + u1(k) + w1(k); // x position, with u1 meaning incremental evolution due to velocity 
        x2(k) = x2(k-1) + u1(k) + w2(k); // y position, with u2 meaning incremental evolution due to velocity 
        y1(k) = sqrt(x1(k)^2+x2(k)^2+v1(k)^2); // range measurement with respect to origin (0,0)
        y2(k) = atan2(x2(k),x1(k)); // angular measurement with respect to X coordinate axis
    */
public:
	BayesianFiltering::Result ProcessModel(Eigen::VectorXd & X, Eigen::VectorXd & U, double t, Eigen::VectorXd & W) override
    {
        //return nonlinear_ProcessModel(X, U, t, W);
        Eigen::VectorXd Xpos(2,1);  
        Xpos(0,0) = X(0,0) + U(0) + W(0,0);
        Xpos(1,0) = X(1,0) + U(1) + W(1,0);
        X = Xpos;
    ...
    {
        //return nonlinear_MeasurementModel(Y, X, t, V);
        Y(0,0) = sqrt(X(0,0)*X(0,0)+X(1,0)*X(1,0)+V(0,0)*V(0,0));
        if (X(0,0)!=0.0){
            Y(1,0) = atan2(X(1,0),X(0,0))+V(1,0);
        }
        else{
            Y(1,0) = 0.0;
        }
    ...
    BayesianFiltering::Result UpdatedhdX(Eigen::VectorXd & X, double t, Eigen::VectorXd & V) override
    {
        if (X(0,0)*X(0,0)+X(1,0)*X(1,0)+V(0,0)*V(0,0) != 0.0){
            dhdX(0, 0) = 0.5*pow(X(0,0)*X(0,0)+X(1,0)*X(1,0)+V(0,0)*V(0,0),-0.5)*2.0*X(0,0); dhdX(0, 1) = 0.5*pow(X(0,0)*X(0,0)+X(1,0)*X(1,0)+V(0,0)*V(0,0),-0.5)*2.0*X(1,0);
        }
        else{
            dhdX(0, 0) = 0; dhdX(0, 1) = 0;
        }
        if (X(0,0)*X(0,0)+X(1,0)*X(1,0) > 0.0){
            dhdX(1, 0) = -X(1,0)/(X(0,0)*X(0,0)+X(1,0)*X(1,0)); dhdX(1, 1) = X(0,0)/(X(0,0)*X(0,0)+X(1,0)*X(1,0));
        }
        else{
            dhdX(1, 0) = 0; dhdX(1, 1) = 0;
        }
        return BayesianFiltering::ResultFormat(true,"");
    }
    // Necessary only for EKF or other filters requiring such derivatives
    ...
    {
        if (X(0,0)*X(0,0)+X(1,0)*X(1,0)+V(0,0)*V(0,0) != 0.0){
            dhdV(0, 0) = 0.5*pow(X(0,0)*X(0,0)+X(1,0)*X(1,0)+V(0,0)*V(0,0),-0.5)*2.0*V(0,0); dhdV(0, 1) = 0;
        }
        else{
            dhdV(0, 0) = 0; dhdV(0, 1) = 0;
        }
        dhdV(1, 0) = 0.0; dhdV(1, 1) = 1.0;
        return BayesianFiltering::ResultFormat(true,"");
    }
};
...
int main(int argc, char** argv)
{
    ...
    if ((demo_name == "ekf")||(demo_name == "ukf")||(demo_name == "sirf")){
        ...
        /* Model 1 Process:  M. Sanjeev Arulampalam, Simon Maskell, Neil Gordon,
            and Tim Clapp, "A Tutorial on Particle Filters for Online
            Nonlinear/Non-Gaussian Bayesian Tracking", IEEE TRANSACTIONS ON SIGNAL
            PROCESSING, VOL. 50, NO. 2, FEBRUARY 2002, pp. 174-188.
            x1(k) = x1(k-1) + 0.5 + w1(k);
            ...
            y2(k) = (x2(k)^2)/20+v2(k);
        */
        /* Model 2 Process:  tracking x y Euclidian coordinates from rho and alpha polar coordinates.
            x1(k) = x1(k-1) + u1(k) + w1(k); // x position, with u1 meaning incremental evolution due to velocity 
            x2(k) = x2(k-1) + u1(k) + w2(k); // y position, with u2 meaning incremental evolution due to velocity 
            y1(k) = sqrt(x1(k)^2+x2(k)^2+v1(k)^2); // range measurement with respect to origin (0,0)
            y2(k) = atan2(x2(k),x1(k)); // angular measurement with respect to X coordinate axis
        */
        ...
        // System models: one used for the filter, another one to simulate the real system
        std::shared_ptr<BayesianFiltering::StateSpaceNonlinearModelBase> pnonlinear_model_filter;
        std::shared_ptr<BayesianFiltering::StateSpaceNonlinearModelBase> pnonlinear_model_real;
        ...
        if (false){}
            ...
            Nx = 2; Nu = 0; Ny = 2;
            pnonlinear_model_real = std::make_unique<NonlinearModel1>();
            pnonlinear_model_real->SetDimensions(Nx,Nu,Ny,Nx,Ny);
            pnonlinear_model_real->Q(0, 0) = 0.001; pnonlinear_model_real->Q(0, 1) = 0.0;
            pnonlinear_model_real->Q(1, 0) = 0.0; pnonlinear_model_real->Q(1, 1) = 2.0;
            pnonlinear_model_real->R(0, 0) = 0.01; pnonlinear_model_real->R(0, 1) = 0.0;
            pnonlinear_model_real->R(1, 0) = 0.0; pnonlinear_model_real->R(1, 1) = 0.05;
            pnonlinear_model_real->X0 << 0.8, -1.5;
            pnonlinear_model_real->P0 << 0.05, 0.00,
                                0.00, 0.20;
        }
        else if (model_name == "model2"){
            Nx = 2; Nu = 2; Ny = 2;
            pnonlinear_model_real = std::make_unique<NonlinearModel2>();
            pnonlinear_model_real->SetDimensions(Nx,Nu,Ny,Nx,Ny);
            pnonlinear_model_real->Q(0, 0) = 0.01; pnonlinear_model_real->Q(0, 1) = 0.0;
            pnonlinear_model_real->Q(1, 0) = 0.0; pnonlinear_model_real->Q(1, 1) = 0.01;
            pnonlinear_model_real->R(0, 0) = pow(0.1, 2.0); pnonlinear_model_real->R(0, 1) = 0.0;
            pnonlinear_model_real->R(1, 0) = 0.0; pnonlinear_model_real->R(1, 1) = pow(2.0 * std::numbers::pi / 180.0, 2.0);
            pnonlinear_model_real->X0 << 10.0, 0.0; // Starts far from origin, due to singularity on (0,0)
            pnonlinear_model_real->P0 << pow(10.0, 2.0), 0.0,
                                0.0, pow(10.0, 2.0);
            pnonlinear_model_real->Y_isangle[1] = true; // y2(k) is polar coordinates angle.
        }
        else{
            printf("\nError: model name=\"%s\" is not valid\n\n",model_name.c_str());
            return -1;
        }

        pnonlinear_model_filter = pnonlinear_model_real;
        ...
        if (demo_name=="ekf"){
            ...
            pfilter->Reset(*pnonlinear_model_filter);    
        }
        if (demo_name=="ukf"){
        ...
        if (demo_name=="sirf"){
            ...
            dynamic_cast<BayesianFiltering::SamplingImportanceResamplingFilter*>(pfilter.get())->Reset(*pnonlinear_model_filter, Nparticles, estimate_type);
        ...
        // Main loop:
        BayesianFiltering::Result res;
        Eigen::VectorXd Xreal = pnonlinear_model_real->X0;
        for (int k=1; k < 1000; ++k){
            double t = k * 0.2;
            ...
            if (false){}
                ...
                U << 0.0, 0.1; // Vehicle evolving, 0.1 m in vertical
            }
            Eigen::VectorXd W(2,1);
            res = BayesianFiltering::MultivariateZeroMeanGaussianSampler(W, pnonlinear_model_real->Q);
            if (!res.first){
                printf("\nError at iteration k=%i: %s",k,res.second.c_str());
            }
            res = pnonlinear_model_real->ProcessModel(Xreal,U,t,W);
            if (!res.first){
                printf("\nError at iteration k=%i: %s",k,res.second.c_str());
            }
            Eigen::VectorXd Y(2,1);
            Eigen::VectorXd V(2,1);
            res = BayesianFiltering::MultivariateZeroMeanGaussianSampler(V, pnonlinear_model_real->R);
            if (!res.first){
                printf("\nError at iteration k=%i: %s",k,res.second.c_str());
            }
            res = pnonlinear_model_real->MeasurementModel(Y,Xreal,t,V);
            if (!res.first){
                printf("\nError at iteration k=%i: %s",k,res.second.c_str());
            }
            ...
            // Prediction:
            auto time_start_predict = std::chrono::steady_clock::now();
            res = pfilter->Prediction(U, t, *pnonlinear_model_filter);
            if (!res.first){
                printf("\nError at iteration k=%i: %s",k,res.second.c_str());
            }
            auto time_stop_predict = std::chrono::steady_clock::now();
            ...
            // Correction:
            auto time_start_correct = std::chrono::steady_clock::now();
            res = pfilter->Correction(Y, t, *pnonlinear_model_filter);
            if (!res.first){
                printf("\nError at iteration k=%i: %s",k,res.second.c_str());
            }
            auto time_stop_correct = std::chrono::steady_clock::now();
        ...
        matplot::figure(f1);
        matplot::subplot(2, 2, 0); matplot::plot(y1, "b-"); matplot::title("y_1(k)"); matplot::ylabel("y_1");
        matplot::subplot(2, 2, 1); matplot::plot(y2, "b-"); matplot::title("y_2(k)"); matplot::ylabel("y_2");
        matplot::subplot(2, 2, 2); matplot::plot(x1_error, "b-"); matplot::hold(true); matplot::plot(x1_error_interval_sup, "r-"); matplot::plot(x1_error_interval_inf, "r-"); matplot::title("x_1 error (blue) and 3sigma interval (red)"); matplot::ylabel("x_1(k)-x_1(k|k)");
        matplot::subplot(2, 2, 3); matplot::plot(x2_error, "b-"); matplot::hold(true); matplot::plot(x2_error_interval_sup, "r-"); matplot::plot(x2_error_interval_inf, "r-"); matplot::title("x_2 error (blue) and 3sigma interval (red)"); matplot::xlabel("k"); matplot::ylabel("x_2(k)-x_2(k|k)");
    ...
    printf("\n    \"ekf model_name\", for extended Kalman filter applied to \"model_name\"");
    printf("\n    \"ukf model_name\", for unscented Kalman filter applied to \"model_name\"");
    printf("\n    \"sirf model_name\", for sampling importance resampling particle applied to \"model_name\" with default number of particles and conditional mean estimate");
    printf("\n    \"sirf model_name Nparticles\", for sampling importance resampling particle applied to \"model_name\" with Nparticles number of particles and conditional mean estimate");
    printf("\n    \"sirf model_name Nparticles MEAN\", for sampling importance resampling particle applied to \"model_name\" with Nparticles number of particles and conditional mean estimate");
    printf("\n    \"sirf model_name Nparticles MAP\", for sampling importance resampling particle applied to \"model_name\" with Nparticles number of particles and maximum a posteriori estimate");
    printf("\nIn above nonlinear filters, \"model_name\" can be \"model1\" or \"model2\"\n");
